import React, { useState } from 'react';
import './layout.css';
import Typography from '@material-ui/core/Typography';
import CssBaseline from '@material-ui/core/CssBaseline';
import { ThemeProvider, Button } from '@material-ui/core';
import { createMuiTheme, makeStyles } from '@material-ui/core/styles';
import Paper from '@material-ui/core/Paper';
import IconButton from '@material-ui/core/IconButton';
import Brightness3Icon from '@material-ui/icons/Brightness3';
import Brightness7Icon from '@material-ui/icons/Brightness7';
import SEO from './seo';
import Image from './image';
import Layout from './layout';
import dict from '../dict';
import { getLanguageFromLS, localStorageKey } from '../helpers';

const useStyles = makeStyles({
  btn: {
    fontWeight: 'bold',
  },
});

const LangBtn = () => {
  const classes = useStyles();

  const langNames = {
    pl: 'pl',
    en: 'en',
  };

  const [isPolish, setIsPolish] = useState(getLanguageFromLS() !== langNames.en);

  return (
    'aaaaaaaaaaaaaaa'
    // <IconButton
    //   aria-label="language"
    //   onClick={() => {
    //     const newLang = isPolish ? langNames.en : langNames.pl;
    //     localStorage.setItem(localStorageKey.language, newLang);
    //     setIsPolish((prevIsPolish) => !prevIsPolish);
    //   }}
    // >
    //   {isPolish
    //     ? <Typography noWrap className={classes.btn} color="primary">{langNames.pl.toUpperCase}</Typography>
    //     : <Typography noWrap className={classes.btn} color="primary">{langNames.en.toUpperCase}</Typography>}
    // </IconButton>
  );
};

export default LangBtn;
